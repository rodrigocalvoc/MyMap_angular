...
@Component({
  ...
  styleUrls: ['./flag-info.component.css']
})
     ...
     if(!this.input.id){
      Swal.fire({
        ...
        this.dataService.searchCountry(this.input.oficialName).pipe(
          map(response => response.map((item: CountriesApiResponse, index:number) => ({id: index +1 , name: item.translations['spa'].common , oficialName: item.translations['spa'].official, population: item.population, capital: item.capital, cryptocurrency: item.currencies, flag: item.flags.svg})))
        ).subscribe((countriesSearch: CountrySearch[]) => {
          this.info = countriesSearch;
          this.bandera = this.info[0].flag;
        });
